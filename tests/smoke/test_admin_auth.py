...
Mirrors the admin-auth shape from the bf twin.
"""
...
def test_admin_lists_logs_across_tenants(admin_app, admin_client, admin_token):
    ...
    admin_client.post("/_twin/accounts", json={"kind": "api_key"}, headers=a_headers)
    admin_client.post("/_twin/accounts", json={"kind": "api_key"}, headers=b_headers)
